...
// injection guard for interpolating the login into a GraphQL query string (dates are generated by
// us, never user input) — NOT GitHub's naming policy. We deliberately do NOT require the login to
// start/end alphanumeric: legacy grandfathered accounts (created before ~2013) end with a hyphen
// or contain double hyphens — e.g. "Link-", "gil--", "p-" are all real accounts — and must still
// be lookup-able. The charset+length bound still blocks quotes, spaces, braces, backslashes, etc.
const LOGIN_RE = /^[a-zA-Z0-9-]{1,39}$/;

/** Whether `login` is safe to interpolate into a GraphQL query (GitHub's login charset + length).
 *  Not GitHub's full naming policy — see LOGIN_RE. Exported for tests. */
